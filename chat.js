/* Studio chat widget — talks to api.josh.menu concierge endpoints.
   Beta gate (off = public): when true, renders only when
   localStorage jm_chat_beta === "1". */
(function () {
  var BETA_GATE = false;
  try {
    if (BETA_GATE && window.localStorage.getItem("jm_chat_beta") !== "1") return;
  } catch (_) {
    return;
  }

  var LOCAL = /^(localhost|127\.0\.0\.1)$/.test(window.location.hostname);
  var HUB = LOCAL ? "http://localhost:3000" : "https://api.josh.menu";
