(function () {
  var HUB_CONFIG = "https://api.josh.menu/webhooks/contact/site_config";
  ...
  fetch(HUB_CONFIG, { credentials: "omit" })
    ...
    .then(function (cfg) {
      if (!cfg || !cfg.configured || !cfg.sitekey) {
        if (statusEl) {
          statusEl.textContent = "Contact form isn’t ready yet — email josh@josh.menu for now.";
        }
        return;
      }
    ...
    .catch(function () {
      if (statusEl) {
        statusEl.textContent = "Couldn’t reach bot protection — email josh@josh.menu.";
      }
    });
